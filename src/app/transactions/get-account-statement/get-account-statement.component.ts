...
@Component({
  selector: 'app-get-account-statement',
  templateUrl: './get-account-statement.component.html',
  styleUrls: ['./get-account-statement.component.scss']
})
export class GetAccountStatementComponent implements OnInit, OnDestroy {
  ...
  statement_detail_form: FormGroup;
  fileName: string;
  ...
  constructor(private customerService: CustomerService, private accountService: AccountService, private excelService: ExcelService) { }
  ...
  onGetStatement(form: NgForm){
	  this.fileName = this.account.accountNumber + this.customer.first_name;
	  var from_dt = new Date(form.value.from_date).getTime();
	  var to_dt = new Date(form.value.to_date).getTime() + 86399999;
	  ...
	  for (var i=0; i< this.transactions.length; i++){
		  var t = new Date(this.transactions[i].time).getTime();;
		  if((t >=  from_dt) && (t <= to_dt)){
	  ...
	  this.excelService.exportAsExcelFile(this.table_data, this.fileName);
  }
